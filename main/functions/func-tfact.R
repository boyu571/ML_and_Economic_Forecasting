...
run.tfact=function(Y,indice,horizon){
  ...
  fmodel=runfact(Y2,indice,horizon=horizon)
...
tfact.rolling.window=function(Y,npred,indice=1,horizon=1){
  ...
  for(i in npred:horizon){
    ...
    fact=run.tfact(Y.window,indice,horizon)
